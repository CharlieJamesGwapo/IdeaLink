# IdeaLink Enhancement Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Upgrade the IdeaLink student-facing experience with a 3-step IPO stepper on SubmitPage, slide animations between steps, and global animation/responsiveness polish.

**Architecture:** Phase 1 rewrites SubmitPage into a stateful 3-step wizard (department → category → form) using local `step` state and CSS-only slide transitions. Phase 2 adds slide keyframe CSS to index.css. Phase 3 adds minor polish to HomePage footer.

**Tech Stack:** React 18, TypeScript, Tailwind CSS v4, Lucide React, existing PHP/MySQL API (unchanged)

---

## Pre-flight: What's Already Done

After reviewing the codebase, several items from the spec are **already implemented**:
- `SuggestionRow` already has both desktop table + mobile card layouts (`hidden md:table-row` pattern)
- `SubmissionsPage` cards already have stagger `animate-fade-in` with `animationDelay`
- `AnnouncementsPage` cards already have stagger `animate-fade-in`
- `input-field` already has orange focus ring (`box-shadow: 0 0 0 3px rgba(244,124,32,0.18)`)
- `stat-card` already has `hover:-translate-y-0.5`
- Skeleton shimmer already applied everywhere

**Real work remaining:**
1. SubmitPage → 3-step IPO stepper (main task)
2. `index.css` → add `slideInRight` keyframe + class for back-navigation animation
3. `HomePage` footer → explicit `grid-cols-1` default

---

## Task 1: Add Slide Animations to index.css

**Files:**
- Modify: `frontend/src/index.css`

- [ ] **Step 1: Add `slideInRight` keyframe and class**

Open `frontend/src/index.css`. After the existing `@keyframes slideInLeft` block (around line 53), add:
...
After the existing `.animate-slide-in-left` line (around line 82), add:
...
Run:
```bash
grep -n "slideInRight\|slideOutLeft" frontend/src/index.css
```
Expected output: lines with the new keyframe names.
...
git commit -m "feat: add slideInRight animation for stepper back-navigation"
```

---

## Task 2: Rewrite SubmitPage with 3-Step IPO Stepper

**Files:**
- Modify: `frontend/src/pages/user/SubmitPage.tsx`

This is the main task. The complete new file replaces the existing flat form with a stateful 3-step wizard.

- [ ] **Step 1: Replace SubmitPage.tsx with the stepper implementation**

Replace the entire contents of `frontend/src/pages/user/SubmitPage.tsx` with:
...
export function SubmitPage() {
  ...
  const Step2 = (
    <div key="step2" className={animClass}>
      <p className="text-center text-gray-400 text-sm font-body mb-6">
        Select the service your feedback is about — <span className="text-ascb-orange font-semibold">{department}</span>
      </p>
...
Run:
```bash
cd /Users/dev3/IdeaLink/frontend
npx tsc --noEmit 2>&1 | head -40
```
Expected: no errors, or only pre-existing errors unrelated to SubmitPage.
...
git commit -m "feat: replace flat SubmitPage with 3-step IPO stepper (department → category → details)"
```

---

## Task 3: Fix HomePage Footer Grid Responsiveness

**Files:**
- Modify: `frontend/src/pages/public/HomePage.tsx`

- [ ] **Step 1: Add explicit `grid-cols-1` to footer grid**

In `frontend/src/pages/public/HomePage.tsx`, find the footer grid div:
...
git commit -m "fix: add explicit grid-cols-1 fallback on homepage footer for mobile"
```
...
- [ ] **Step 2: Test Step 1 — Department selection**

Navigate to the Submit Feedback page (log in as a student first).
- Verify two large cards appear: "Registrar" and "Accounting Office"
- Click "Registrar" — card highlights orange, checkmark appears, auto-advances to Step 2 after brief delay
- Progress bar: step 1 is orange, step 2 is grey

- [ ] **Step 3: Test Step 2 — Category grid**

- Verify 8 category buttons appear in a grid
- Click any button — it highlights orange
- Click "Back" — slides right back to Step 1, department card stays selected
- Select "Accounting Office" — category grid resets, shows 8 accounting categories
- Select a category, click "Continue" — advances to Step 3

- [ ] **Step 4: Test Step 3 — Details form**

- Verify summary pills show selected department and category at top
- Verify OfficeHoursBanner loads
- Fill in Subject and Details
- Toggle Anonymous — yellow warning appears
- Click "Back" — slides right to Step 2, category still selected
- Click "Submit Feedback" — success screen appears with department + category shown

- [ ] **Step 5: Test success screen reset**

- Click "Submit Another" — resets to Step 1, no pre-selected department

- [ ] **Step 6: Test on mobile viewport**

Resize browser to 375px wide (iPhone SE).
- Verify Step 1 cards stack vertically (full width)
- Verify Step 2 category grid shows 2 columns
- Verify progress bar shows dots only (no labels on small screens)
- Verify no horizontal scroll

- [ ] **Step 7: Test homepage footer on mobile**

Navigate to `/` and resize to 375px.
- Verify footer columns stack vertically with proper spacing

- [ ] **Step 8: Stop dev server when done**
...
After all tasks complete, the git log should show:
1. `feat: add slideInRight animation for stepper back-navigation`
2. `feat: replace flat SubmitPage with 3-step IPO stepper (department → category → details)`
3. `fix: add explicit grid-cols-1 fallback on homepage footer for mobile`
