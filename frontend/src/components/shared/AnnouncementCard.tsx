...
export function AnnouncementCard({ announcement }: Props) {
  ...
  const isRecent =
    Date.now() - new Date(announcement.date_posted).getTime() < 3 * 24 * 60 * 60 * 1000
  ...
  return (
    <div className="group relative glass rounded-2xl p-5 hover:border-ascb-orange/30 transition-all duration-300 hover:-translate-y-0.5 border border-transparent">
      {/* Orange left accent */}
      <div className="absolute left-0 top-4 bottom-4 w-0.5 bg-gradient-to-b from-ascb-orange to-ascb-gold rounded-full opacity-0 group-hover:opacity-100 transition-opacity" />

      <div className="flex items-start gap-4">
        <div className="shrink-0 w-10 h-10 rounded-xl bg-ascb-orange/15 flex items-center justify-center group-hover:bg-ascb-orange/25 transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-gray-400 text-sm leading-relaxed font-body">{announcement.message}</p>
