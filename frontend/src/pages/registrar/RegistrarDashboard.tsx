...
export function RegistrarDashboard() {
  ...
  useEffect(() => {
    getOfficeHours('Registrar').then(res => setOfficeHoursState(res.data)).catch(() => {})
  }, [])
  ...
  const handleSetOfficeHours = async (isOpen: boolean) => {
    setIsUpdating(true)
    try {
      const res = await setOfficeHours('Registrar', {
        is_open: isOpen,
    ...
    <div className="animate-fade-in space-y-6 pb-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-white font-display">Registrar Dashboard</h1>
          </div>
          <p className="text-gray-500 text-sm font-ui ml-3">Feedback Analytics · Registrar Office</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => exportToCSV(suggestions, 'registrar-feedback')}
      ...
      <div className="bg-ascb-navy rounded-2xl border border-ascb-navy-mid overflow-hidden">
        <div className="px-5 py-4 border-b border-ascb-navy-mid flex items-center justify-between">
          ...
          <a href="/registrar/suggestions" className="text-xs text-ascb-orange hover:underline font-ui">View all →</a>
        </div>
