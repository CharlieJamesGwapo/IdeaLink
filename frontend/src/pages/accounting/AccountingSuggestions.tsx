    ...
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Accounting Office — {suggestions.length} submissions</p>
        </div>
        <div className="flex items-center gap-3">
          <button
            onClick={() => exportToCSV(filtered, 'accounting-suggestions')}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 glass rounded-2xl">
          ...
          <p className="text-gray-600 text-sm mt-1">Suggestions for Accounting Office will appear here</p>
        </div>
      ) : (
        <div className="bg-navy rounded-2xl border border-navy-light overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((s) => <SuggestionRow key={s.id} suggestion={s} showActions onStatusChange={handleStatusChange} />)}
            </tbody>
          </table>
        </div>
      )}
