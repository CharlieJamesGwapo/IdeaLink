...
func main() {
	...
	// Registrar account
	_, err = db.Exec(
		`INSERT INTO registrar_accounts (username, password) VALUES ($1, $2) ON CONFLICT (username) DO UPDATE SET password = $2`,
		"registrar", staffPass,
	)
	if err != nil {
		log.Fatal(err)
	...
	// Accounting account
	_, err = db.Exec(
		`INSERT INTO accounting_accounts (username, password) VALUES ($1, $2) ON CONFLICT (username) DO UPDATE SET password = $2`,
		"accounting", staffPass,
	)
	if err != nil {
		log.Fatal(err)
	}
	...
	samples := []sampleFeedback{
		{studentID1, "Registrar", "Transcript of Records", "Slow TOR Processing", "It takes more than 2 weeks to process transcript requests. Please improve turnaround time.", "Pending", false, 1},
		{studentID1, "Accounting Office", "Tuition Fee Payment", "Online Payment Option", "Please add GCash or Maya payment options for tuition. Going to the cashier is inconvenient.", "Under Review", false, 3},
		{studentID2, "Registrar", "Enrollment/Registration", "Enrollment System Error", "The online enrollment portal shows error 404 every Monday morning during peak enrollment hours.", "Resolved", false, 5},
		{studentID2, "Accounting Office", "Scholarship/Financial Aid", "Scholarship Notification Delay", "Scholarship recipients are notified very late. I almost missed the deadline for my scholarship application.", "Pending", true, 2},
		{studentID3, "Registrar", "Certificate of Enrollment", "Certificate Request Process", "The process for requesting enrollment certificates should be available online, not just in-person.", "Under Review", false, 7},
		{studentID3, "Accounting Office", "Fee Assessment", "Wrong Assessment Amount", "My assessment shows incorrect tuition amount. I was charged for units I dropped.", "Pending", false, 4},
		{studentID1, "Registrar", "ID Issuance", "ID Replacement Takes Too Long", "It takes 3 months to replace a lost ID. Can this be expedited?", "Resolved", false, 10},
		{studentID2, "Accounting Office", "Clearance Processing", "Clearance Requirement Confusion", "The requirements for clearance are not clearly posted anywhere. Please put them on the school website.", "Pending", true, 6},
		{studentID3, "Registrar", "Good Moral Certificate", "Good Moral Certificate Availability", "Good moral certificates should be available within 3 days, not 2 weeks.", "Pending", false, 8},
		{studentID1, "Accounting Office", "Refund Request", "Refund Processing Time", "Refund requests take over a month to process. Please improve this for students who need urgent funds.", "Under Review", false, 9},
	}
	...
	fmt.Println("Registrar:  registrar / Staff@123")
	fmt.Println("Accounting: accounting / Staff@123")
}
