...
if (!isLoggedIn('registrar_id')) {
    redirect('login.php');
}

// --- ANALYTICS (Registrar only) ---
$total_records = $conn->query("SELECT COUNT(*) as total FROM suggestions WHERE department = 'Registrar'")->fetch_assoc()['total'];
$pending_count = $conn->query("SELECT COUNT(*) as total FROM suggestions WHERE department = 'Registrar' AND (status != 'Reviewed' OR status IS NULL)")->fetch_assoc()['total'];
$student_count = $conn->query("SELECT COUNT(*) as total FROM suggestions WHERE department = 'Registrar' AND user_role = 'Student'")->fetch_assoc()['total'];
$faculty_count = $conn->query("SELECT COUNT(*) as total FROM suggestions WHERE department = 'Registrar' AND user_role = 'Faculty Staff'")->fetch_assoc()['total'];

// Recent registrar activity
$activities = [];
$act_res = $conn->query("SELECT title, date_submitted as dt FROM suggestions WHERE department = 'Registrar' ORDER BY date_submitted DESC LIMIT 5");
if ($act_res) {
...
if (isset($_GET['mark_id'])) {
    ...
    $stmt = $conn->prepare("UPDATE suggestions SET status = 'Reviewed' WHERE id = ? AND department = 'Registrar'");
...
<head>
    ...
    <title>IdeaLink | Registrar Dashboard</title>
    ...
    <div id="dashboard" class="content-section active">
        <div class="content-header">
            <div class="welcome-text">
                ...
                <p>Academic & Record Management</p>
            </div>
        ...
        <div class="card">
            <h3>Recent Registrar Activity</h3>
